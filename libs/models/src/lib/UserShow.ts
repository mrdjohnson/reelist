...
type VideoTableTypeOrEmpty = VideoTableType | EmptyObject
...
class UserShow extends AbstractUserVideo {
  ...
  override toggleWatched = async (isWatchedOverride: boolean | null = null) => {
    ...
    const isTv = this.tmdbVideo.isTv
    ...
    if (nextIsWatched) {
      ...
      if (this.tmdbVideo.nextEpisodeToAir) {
        this.backfillWatched(this.tmdbVideo.lastEpisodeToAir)
      ...
      lastWatchedEpisodeData = episodeToEpisodeWatchedData(this.tmdbVideo.lastEpisodeToAir)
    } else {
  ...
  toggleSeasonWatched = async (
    ...
    const lastEpisodeInSeason = seasonNumber === this.tmdbVideo.lastEpisodeToAir?.seasonNumber
    ...
    if (seasonWillBeWatched) {
      ...
      if (lastEpisodeInSeason) {
        episode = this.tmdbVideo.lastEpisodeToAir
      } else {
  ...
  fetchSeason = async (seasonNumber: number) => {
    ...
    let season: TmdbTvSeason | undefined

    try {
      season = await callTmdb<TmdbTvSeason>(path).then(item => _.get(item, 'data.data'))
    } catch (e) {
      console.error(e)
      throw e
    } finally {
  ...
  override compareCompletionTo(otherVideo: UserShow | UserMovie) {
    ...
    if (this.isCompleted) return otherVideo.isCompleted ? EQUAL_TO : MORE_COMPLETE

    // they cannot watch anymore episodes, and we can
    if (otherVideo.isCompleted) return LESS_COMPLETE

    // they cant watch any episodes today, can we?
  ...
  get lastWatchedSeasonNumber() {
    return 1 //this.lastWatchedEpisodeFromEnd?.seasonNumber
  }

  get lastWatchedEpisodeNumber() {
    return 1 //this.lastWatchedEpisodeFromEnd?.episodeNumber
  }

  get lastEpisodeToAir() {
    if (!this.tmdbVideo.isTv) return

    return this.tmdbVideo.lastEpisodeToAir
  }

  get lastWatchedEpisodeFromEnd() {
    // if (!this.tmdbVideo.isTv) return
    // let lastEpisode = this.tmdbVideo.lastEpisodeToAir
    //
  ...
  // todo: re-calc based on watched episode count of season
  get isLatestEpisodeWatched() {
    if (!this.tmdbVideo.isTv || !this.lastEpisodeToAir) return this.isWatched
  ...
  override get isCompleted() {
    ...
    if (this.tmdbVideo.isTv && this.tmdbVideo.nextEpisodeToAir) return false
  ...
  get minEpisodeRunTime() {
    if (!this.tmdbVideo.isTv) return 0
